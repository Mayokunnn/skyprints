  ...
  switch (action.type) {
    case "ADD_ITEM": {
      ...
      if (existingItemIndex > -1) {
        ...
        newItems = state.items.map((item, index) => (index === existingItemIndex ? action.payload : item))
      } else {
...
export function CartProvider({ children }: { children: React.ReactNode }) {
  ...
  const getWhatsAppMessage = () => {
    ...
    state.items.forEach((item, index) => {
      ...
      if (item.options.material) message += `   🏗️ Material: ${item.options.material}\n`
      if (item.options.finish) message += `   ✨ Finish: ${item.options.finish}\n`
    ...
    message += "Please provide me with a detailed quote and timeline for this order. Thank you!"
